...
import { EditProduct } from "services/operations/Product";
...
export default function ProductFormEdit(props) {
  ...
  useEffect(() => {
    if (product) {
      setFileLinks(product?.image || []); 
      setActive(product?.active ?? true); 
      setBestSelling(product?.bestSelling ?? false);
      setPopular(product?.popular ?? false);
      setIsVariant(product?.isVariant ?? false);
  
      // Convert variants to the required format
      if (product?.variants) {
        const formattedVariants = product.variants.flatMap(variant =>
          variant.options.map(option => ({
            type: variant.type,
            quantity: option.quantity.toString(), // Ensure it's a string
            price: option.price,
            makingPrice: option.makingPrice
          }))
        );
  ...
  return <Card className="p-3">
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ sm: 6, xs: 12 }}>
          <Autocomplete
            ...
            value={categories.find((cat) => cat._id === methods.getValues("category")) || null} // ✅ Match selected category
    ...
    {fileLinks.map((link, index) => (
      <UploadImageBox key={index}>
        <Box component="img" src={link} width="100%" />
        <StyledClear onClick={() => handleRemoveImage(index)} />
      </UploadImageBox>
    ))}

    {/* ✅ Show New Uploaded Images */}
    {files.map((file, index) => (
      <UploadImageBox key={index + fileLinks.length}>
